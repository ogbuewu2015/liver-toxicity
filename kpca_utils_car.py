...
# === kpca_training_and_saving.py ===
import pandas as pd
from sklearn.decomposition import KernelPCA
import joblib

# 👇 Patched KernelPCA to expose gamma_ if needed
class PatchedKernelPCA(KernelPCA):
    @property
    def gamma_(self):
        return self.gamma

# Your existing split function
def split_columns(df):
    float_cols = df.select_dtypes(include=['float64', 'float32']).columns
    non_float_cols = df.select_dtypes(exclude=['float64', 'float32']).columns
    return df[float_cols], df[non_float_cols]

def fit_kernel_pca(float_df, n_components=30, kernel='rbf', **kwargs):
    kpca = PatchedKernelPCA(n_components=n_components, kernel=kernel, **kwargs)
